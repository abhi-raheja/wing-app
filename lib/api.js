 ...
 * @returns {Promise<string>} Provider key (anthropic, openai, google)
 */
 ...
 */
async function makeGoogleRequest({ apiKey, prompt, systemPrompt, maxTokens, controller }) {
  const config = PROVIDERS.google;
  const url = `${config.apiUrl}/${config.model}:generateContent?key=${apiKey}`;
  ...
  try {
    ...
    switch (provider) {
      ...
      case 'openai':
        ...
        break;
      case 'google':
        result = await makeGoogleRequest(requestArgs);
        break;
      default:
